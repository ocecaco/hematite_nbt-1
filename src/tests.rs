...
fn nbt_nonempty() {
    ...
    // Test correct length.
    let mut dst = Vec::new();
    nbt.to_writer::<_, BigEndian>(&mut dst).unwrap();
    ...
    let file = Blob::from_reader::<_, BigEndian>(&mut src).unwrap();
    assert_eq!(&file, &nbt);
}
...
fn nbt_no_root() {
    ...
    assert_eq!(Blob::from_reader::<_, BigEndian>(&mut io::Cursor::new(&bytes[..])),
...
fn nbt_bad_compression() {
    ...
    assert!(Blob::from_gzip_reader::<_, BigEndian>(&mut io::Cursor::new(&bytes[..])).is_err());
    assert!(Blob::from_zlib_reader::<_, BigEndian>(&mut io::Cursor::new(&bytes[..])).is_err());
}
...
fn nbt_compression() {
    ...
    nbt.to_zlib_writer::<_, BigEndian>(&mut zlib_dst).unwrap();
    let zlib_file = Blob::from_zlib_reader::<_, BigEndian>(&mut io::Cursor::new(zlib_dst)).unwrap();
    assert_eq!(&nbt, &zlib_file);

    // Test gzip encoding/decoding.
    let mut gzip_dst = Vec::new();
    nbt.to_gzip_writer::<_, BigEndian>(&mut gzip_dst).unwrap();
    let gz_file = Blob::from_gzip_reader::<_, BigEndian>(&mut io::Cursor::new(gzip_dst)).unwrap();
...
fn nbt_bigtest() {
    ...
    let bigtest = Blob::from_gzip_reader::<_, BigEndian>(&mut bigtest_file).unwrap();
    // This is a pretty indirect way of testing correctness.
    let mut dst = Vec::new();
    bigtest.to_writer::<_, BigEndian>(&mut dst).unwrap();
...
fn nbt_arrays() {
    ...
    let arrays = Blob::from_reader::<_, BigEndian>(&mut arrays_file).unwrap();
    match &arrays["ia"] {
...
fn serde_blob() {
    ...
    let file: Blob = from_reader::<_, _, BigEndian>(&mut src).unwrap();
    assert_eq!(&file, &nbt);
    let mut dst = Vec::new();
    to_writer::<_, _, BigEndian>(&mut dst, &nbt, None).unwrap();
    // We can only test if the decoded bytes match, since the HashMap does
    // not guarantee order (and so encoding is likely to be different, but
    // still correct).
    assert_eq!(&bytes.len(), &dst.len());
