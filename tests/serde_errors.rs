...
fn no_root_compound() {
    ...
    let mut dst = Vec::new();
    let write = to_writer::<_, _, BigEndian>(&mut dst, &nbt, None);
...
fn incomplete_nbt() {
    ...
    let read: Result<ByteNbt> = from_reader::<_, _, BigEndian>(&bytes[..]);
...
fn non_boolean_byte() {
    ...
    let read: Result<BoolNbt> = from_reader::<_, _, BigEndian>(&bytes[..]);
